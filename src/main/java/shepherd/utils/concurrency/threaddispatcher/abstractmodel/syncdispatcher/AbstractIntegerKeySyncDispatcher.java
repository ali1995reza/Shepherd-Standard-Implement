...
public abstract class AbstractIntegerKeySyncDispatcher <T , QT>
        implements SynchronizedDispatcher<Integer,T> ,
        DispatchQueueMethods<Integer , T , QT> {
    ...
    private SyncConsumer<Integer , T> consumer;
    private final ArrayBlockingDeque<SyncDispatchSequence<Integer , T , QT>> queue;
    private ThreadFactory threadFactory;
    private final Object _sync = new Object();
    private boolean isActive = false;
    private boolean isTerminated = false;
    private CountDownLatch stopLatch = null;
    private final SyncDispatchSequence<Integer , T , QT>[] sequences;
    ...
    public AbstractIntegerKeySyncDispatcher(int numberOfThreads , SyncConsumer<Integer , T> co , ThreadFactory factory)
    {
        ...
        sequences = new SyncDispatchSequence[100000+1];
        numberOfHandlerThreads = numberOfThreads;
    }

    public AbstractIntegerKeySyncDispatcher(int numberOfThreads , SyncConsumer<Integer , T> co)
    {
    ...
    public AbstractIntegerKeySyncDispatcher(SyncConsumer<Integer , T> co)
    {
        this(1,co);
    }
    ...
    @Override
    public void dispatch(Integer syncId, T data) throws InterruptedException {


        assertIfTerminated();

        SyncDispatchSequence<Integer, T, QT> sequence = getOrCreateSequence(syncId);
    ...
    @Override
    public boolean tryDispatch(Integer syncId, T data) {
    ...
    private SyncDispatchSequence<Integer , T , QT> getOrCreateSequence(Integer i)
    {
        ...
        if(sequence==null)
        {
            synchronized (_sync)
            {
                ...
                {
                    sequences[i] =
                            new SyncDispatchSequence<Integer, T, QT>(this , i);
                    createdIds.add(i);
                }
    ...
    private void mainLoop()
    {
        ...
        while (true) {
            try {
                ...
                SyncDispatchSequence<Integer , T , QT> sequence = queue.takeFirst();
    ...
    protected abstract void computeData(Integer syncId , QT queueData , SyncConsumer<Integer , T> consumer);


    @Override
    public Map<Integer , List<T>> terminate() {
        synchronized (_sync)
        {
            assertIfTerminated();
        ...
        Map<Integer , List<T>> remaining = getRemainingData();
    ...
    @Override
    public void setConsumer(SyncConsumer<Integer , T> consumer) {
        assertIfTerminated();
    ...
    private final Map<Integer , List<T>> getRemainingData()
    {
        HashMap<Integer , List<T>> remaining = new HashMap<>();


        for (Integer key :createdIds)
        {
            SyncDispatchSequence<Integer , T, QT> sequence = sequences[key];
    ...
    protected abstract List<T> getRemainingData(Integer syncId , Deque<QT> queue);
    ...
    @Override
    public final void addToHandlerQueue(SyncDispatchSequence<Integer, T, QT> s) {

        queue.addLast(s);
    }


    protected abstract boolean putInQueue(Integer id, T data, Deque<QT> queue);

    @Override
    public final boolean putInSyncQueue(Integer id, T data, Deque<QT> queue) {
    ...
    private final void handleUntilLatchActive(CountDownLatch conditionLatch)
    {
        ...
        while (conditionLatch.getCount()>0) {
            try {
                ...
                SyncDispatchSequence<Integer , T , QT> sequence = queue.pollFirst(10 , TimeUnit.MILLISECONDS);
